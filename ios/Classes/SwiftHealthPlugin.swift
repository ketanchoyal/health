...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
    ...
    func writeData(call: FlutterMethodCall, result: @escaping FlutterResult) throws {
        ...
        var meta = [String: Any]()
        let metaDataKeySyncIdentifier = (arguments["metaDataKeySyncIdentifier"] as? String)
        if (metaDataKeySyncIdentifier != nil) {
            meta[HKMetadataKeySyncVersion] = 1
            meta[HKMetadataKeySyncIdentifier] = metaDataKeySyncIdentifier
        }
        
        let sample: HKObject
        
        if (unitLookUp(key: type) == HKUnit.init(from: "")) {
            sample = HKCategorySample(type: dataTypeLookUp(key: type) as! HKCategoryType, value: Int(value), start: dateFrom, end: dateTo, metadata: meta.isEmpty ? nil : meta)
        } else {
            let quantity = HKQuantity(unit: unitDict[unit]!, doubleValue: value)
            
            sample = HKQuantitySample(type: dataTypeLookUp(key: type) as! HKQuantityType, quantity: quantity, start: dateFrom, end: dateTo,metadata: meta.isEmpty ? nil : meta)
        }
    ...
    func deleteData(call: FlutterMethodCall, result: @escaping FlutterResult) throws {
        ...
        let predicate = HKQuery.predicateForObjects(withMetadataKey: HKMetadataKeySyncIdentifier, allowedValues: [metaDataKeySyncIdentifier])
        
        HKHealthStore().deleteObjects(of: dataTypeLookUp(key: type), predicate: predicate) { success, _, error in
            if let err = error {
                print("Error Deleting \(type) Sample: \(err.localizedDescription), Id: \(metaDataKeySyncIdentifier)")
            }
            DispatchQueue.main.async {
                result(success)
            }
